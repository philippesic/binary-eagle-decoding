...
class NativeContractTests(unittest.TestCase):
    ...
    def test_output_matches_packed_f32_reference_then_bf16(self):
        for k in (1, 31, 33, 65):
            with self.subTest(k=k):
                ...
                expected = binary_linear(
                    ...
                    pack_weights(weights.tolist()),
                    bias=bias.tolist(),
                )
                expected_bf16 = torch.tensor(expected, dtype=torch.float32).bfloat16()
                self.assertTrue(
                    torch.equal(native_contract_linear(inputs, weights, bias), expected_bf16)
                )
