...
- Pause procedure: local `scripts/agent_env.py pause rtx2080ti`, interrupt active remote supervisor through tmux MCP, wait for stopped state, inspect all recorded process groups and explicit WSL nvidia-smi before declaring GPU free. Nested diagnostic launcher records its current child in `progress.json`; confirm that child/server also stops.
...
- `/root/gpu_monitor` owns the GPU and proceeds sequentially; root only reads/postprocesses completed artifacts. No final prompts or training are used. On failure or user pause, stop the outer supervisor and the active child/server process groups recorded by the launcher, then verify GPU idle before reporting it free.
